...
/*
 * The following CRC-16 code is based closely off code from the Linux kernel
 */
...
Header* tcpheader_create(uint16_t sport, uint16_t dport, uint32_t seqnum,
	...
	memcpy(packet+TCP_HEADER_SIZE, data, data_bytes);

	uint16_t crc = crc16(0, packet, data_bytes+TCP_HEADER_SIZE);
	h->field.checksum = htons(crc);
...
int tcpheader_verifycrc(char *packet, int bytes) {
	...
	h->field.checksum = 0;
	uint16_t crc = crc16(0, packet, bytes);
	return (crc == sent_crc);
}
